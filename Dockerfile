...
RUN wget https://why3.gitlabpages.inria.fr/releases/why3-1.5.0.tar.gz
RUN tar zxf why3-1.5.0.tar.gz && rm why3-1.5.0.tar.gz
COPY resources/why3-150-fix/isabelle.ml why3-1.5.0/src/printer/isabelle.ml
# COPY resources/why3.ML.2021-1 why3-1.5.0/lib/isabelle/why3.ML.2021-1
RUN cd why3-1.5.0 && ./configure && make
RUN ISAINSTDIR=`ls -d /usr/local/Isabelle*`; echo "/usr/local/lib/why3/isabelle" >> ${ISAINSTDIR}/etc/components

RUN cd why3-1.5.0; make install; make byte; make install-lib ; cd ..; rm -r why3-1.5.0
